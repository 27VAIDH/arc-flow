...
export default function WorkspaceManager({
  ...
  const handleCreate = useCallback(async () => {
    try {
      ...
      // Ignore errors (e.g., empty name shouldn't happen here)
    }
  }, []);
    ...
    <div className="px-3 py-2">
      ...
      <div className="space-y-1">
        {workspaces.map((ws) => (
          ...
          >
            {/* Emoji with accent color background */}
            <button
              onClick={() => setShowEmojiPicker(ws.id)}
              className="w-7 h-7 rounded-md flex items-center justify-center text-sm shrink-0"
              style={{ backgroundColor: ws.accentColor + "20" }}
              title="Change emoji"
            >
              {ws.emoji}
            </button>
            ...
            {editingId === ws.id ? (
              <input
                ...
                className="flex-1 text-sm bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-500 rounded px-1.5 py-0.5 outline-none focus:ring-1 focus:ring-blue-500"
              />
            ) : (
              <span
            ...
            {/* Color indicator */}
            <button
              onClick={() => setShowColorPicker(ws.id)}
              className="w-3 h-3 rounded-full shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
      ...
      {showEmojiPicker && (
        <div
          ref={emojiPickerRef}
          className="mt-1 p-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg"
        >
